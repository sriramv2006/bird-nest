...
// Arbitrary value set by user
float targetWheelSpeed = 3;
// Current motor setting
int motorspeed = 0;
// # of slits on the encoder disk
float numDiskSlots = 16;
// # of rotations of the encoder disk to spin the wheel 1 revolution
float numDiskRevPerWheelRev = 33;
// The last measured wheel revolutions per second
float lastSpeed = 0.0;
// The last increment in motor setting speed
int lastChange = 0;
// User-setting for max increase in speed per sample cycle
int MAX_INCREASE = 40;
AF_DCMotor motor(4);
...
// Input = max delay of script, how long to wait before no movement assumption
// Output = seconds per 1 slit
...
// Input = current wheel revs per sec
// Output = motor value change
int calcSpeed(float currentSpeed) {  
  // change in speed from last sample cycle
  float speedDiff = currentSpeed - lastSpeed;
  // the speed of the wheel if we maintained the same increase
